...
import { EditOwnerUser } from "./EditOwnerUser";

interface ActionBlogProps{
  eventProfile:EventProfile | null;
  user_id:string | undefined;
}
const ActionBlog:React.FC<ActionBlogProps> = ({ eventProfile, user_id }) => {
  const dispatch = useAppDispatch(); 
  let inEventTime = 0;
  if(eventProfile){
    inEventTime = Math.floor(
  ...
  return (
    <>
      {/*inEventTime <= currentTime ? (
        ...
        ) :*/} {eventProfile&&eventProfile.ownerUser === user_id ? (
        <EditOwnerUser eventProfile={eventProfile} />
      ) :eventProfile?.users&& eventProfile.users.find((u) => u === user_id) ? (
